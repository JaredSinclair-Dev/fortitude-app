...
const C = {
  ...
  text: "#dce6f5", textMuted: "#b8c8d8", textDim: "#7a8fa8",
};
...
const TradingSessionsMap = () => {
  ...
  const getState = (s) => {
    ...
    // FX market closes Friday 22:00 UTC, opens Sunday 22:00 UTC
    const isFxWeekend = (dayUTC === 6) || (dayUTC === 0 && utc < 22) || (dayUTC === 5 && utc >= 22);
    const isOpen  = !isFxWeekend && utc >= s.openUTC && utc < s.closeUTC;
    ...
    const weekendMsg = isFxWeekend ? "Markets closed" : null;
    return { isOpen, pct, color, cityTime: getCityTime(s.tz), countdown: isOpen ? fmtSecs(remain) : fmtSecs(tillOpn), weekendMsg };
  };
  ...
  const getKZState = (kz) => {
    ...
    const isFxWeekend = (dayUTC === 6) || (dayUTC === 0 && utc < 22) || (dayUTC === 5 && utc >= 22);
    const isLive = !isFxWeekend && utc >= kz.open && utc < kz.close;
  ...
  return (
    <div className="mc" style={{ padding:"14px 18px", marginBottom:14 }}>
      ...
      {/* D3 World Map — compact */}
      <div style={{ borderRadius:6, overflow:"hidden", background:"rgba(6,8,16,.7)", border:`1px solid ${C.border}`, position:"relative", marginBottom:14, maxHeight:180 }}>
        {!mapReady && (
          <div style={{ padding:"20px", textAlign:"center", fontSize:12, color:C.textDim }}>Loading map…</div>
        )}
        <svg ref={svgRef} style={{ width:"100%", height:"auto", maxHeight:180, display: mapReady ? "block" : "none" }}/>
      </div>
    ...
    <div className="fi">
      <div className="dash-header" style={{ marginBottom: 20, display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:16 }}>
        <div>
          ...
          <p style={{ color: C.textMuted, fontSize: 14 }}>Structured analysis. Probabilistic framing. Institutional methodology.</p>
        </div>
        {/* User location clock */}
      ...
      <div className="dash-grid" style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) 300px", gap:14, alignItems:"start" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {/* Journal Summary Widget */}
          {(() => {
            const trades = TRADE_DATA;
            const wins   = trades.filter(t => t.win).length;
            const losses = trades.filter(t => !t.win).length;
            const winRate = trades.length > 0 ? Math.round((wins / trades.length) * 100) : 0;
            const totalR  = trades.reduce((a, t) => a + parseFloat(t.result), 0);
            const avgR    = trades.length > 0 ? (totalR / trades.length).toFixed(2) : "0.00";
            const bestTrade  = trades.reduce((b, t) => parseFloat(t.result) > parseFloat(b.result) ? t : b, trades[0]);
            const worstTrade = trades.reduce((w, t) => parseFloat(t.result) < parseFloat(w.result) ? t : w, trades[0]);
            const recentTrades = [...trades].sort((a,b) => new Date(b.time) - new Date(a.time)).slice(0, 5);
            const profitFactor = (() => {
              const gross_profit = trades.filter(t=>t.win).reduce((a,t)=>a+parseFloat(t.result),0);
              const gross_loss   = Math.abs(trades.filter(t=>!t.win).reduce((a,t)=>a+parseFloat(t.result),0));
              return gross_loss > 0 ? (gross_profit / gross_loss).toFixed(2) : "∞";
            })();
            return (
              <div className="mc" style={{ padding:"16px 20px" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
                  <div className="sl" style={{ margin:0 }}>Journal Summary</div>
                  <button className="btn bg" style={{ fontSize:10, padding:"4px 10px" }} onClick={() => setPage("journal")}>View Journal →</button>
                </div>
                {/* KPI row */}
                <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:10, marginBottom:14 }}>
                  {[
                    { l:"Win Rate",      v:`${winRate}%`,        c: winRate >= 50 ? C.accent : C.pink },
                    { l:"Total R",       v:`${totalR >= 0 ? "+" : ""}${totalR.toFixed(1)}R`, c: totalR >= 0 ? C.accent : C.pink },
                    { l:"Avg R/Trade",   v:`${avgR}R`,           c: parseFloat(avgR) >= 0 ? C.accent : C.pink },
                    { l:"Profit Factor", v:profitFactor,         c: parseFloat(profitFactor) >= 1 ? C.accent : C.pink },
                    { l:"Total Trades",  v:trades.length,        c: C.text },
                  ].map(m => (
                    <div key={m.l} style={{ textAlign:"center", padding:"10px 6px", background:"rgba(13,16,24,.6)", borderRadius:6, border:`1px solid ${C.border}` }}>
                      <div className="mn" style={{ fontSize:16, color:m.c, lineHeight:1.1 }}>{m.v}</div>
                      <div style={{ fontSize:10, color:C.textDim, marginTop:3, letterSpacing:".04em", textTransform:"uppercase" }}>{m.l}</div>
                    </div>
                  ))}
                </div>
                {/* Recent trades */}
                <div style={{ fontSize:11, color:C.textDim, letterSpacing:".06em", textTransform:"uppercase", marginBottom:8 }}>Recent Trades</div>
                <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                  {recentTrades.map(t => (
                    <div key={t.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"6px 10px", background:"rgba(13,16,24,.4)", borderRadius:5, border:`1px solid ${C.border}` }}>
                      <span style={{ fontSize:11, color:C.text, fontWeight:500, minWidth:70 }}>{t.instrument}</span>
                      <span style={{ fontSize:10, padding:"2px 6px", borderRadius:3, background:t.type==="Long"?"rgba(41,168,255,.1)":"rgba(233,30,167,.1)", color:t.type==="Long"?C.accent:C.pink, border:`1px solid ${t.type==="Long"?C.accentDim:C.pinkDim}` }}>{t.type}</span>
                      <span style={{ fontSize:11, color:C.textDim, flex:1 }}>{t.session}</span>
                      <span className="mn" style={{ fontSize:12, color:parseFloat(t.result)>=0?C.accent:C.pink, fontWeight:600 }}>{parseFloat(t.result)>=0?"+":""}{t.result}R</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="mc" style={{ borderLeft:`3px solid ${C.accent}` }}>
            ...
            <p style={{ color:C.textMuted,lineHeight:1.8,fontSize:14 }}>DXY continues to exhibit compression between 103.40 and 104.20. Structural bias remains indeterminate until a confirmed break with sufficient momentum. Watch for liquidity sweep below 103.40 as a potential institutional positioning mechanism.</p>
          </div>
    ...
    <div className="fi">
      ...
      {[{t:"Market Structure Assessment",c:C.accent,b:"Current structural bias presents as transitional. Price action has demonstrated a sequence of lower highs since the most recent swing high, however a prior higher low remains intact on the selected timeframe. This creates structural ambiguity that should not be resolved through assumption. A confirmed break above the most recent lower high would shift bias toward bullish continuation."},{t:"Liquidity & Order Flow Context",c:C.accent,b:"Equal lows are present at the base of the most recent consolidation range, representing a probable stop cluster for long positions initiated at that level. Inducement behavior is visible on the immediate left side of current price — a shallow push designed to trigger premature directional entries."},{t:"Risk Architecture Consideration",c:C.gold,b:"Logical invalidation resides below the most recent structural low. Volatility context is compressive — expansion has not been confirmed. This environment does not favor directional commitment. Asymmetric risk definition is possible only if structure confirms at the referenced invalidation boundary."},{t:"Scenario Mapping",c:C.accent,b:"Primary Scenario: Structural continuation requires a break above the most recent lower high with closing price confirmation.\n\nSecondary Scenario: Failure case involves continuation below the equal lows. Should that occur, the structural framework shifts to bearish continuation."},{t:"Behavioral Discipline Reminder",c:C.textMuted,b:"Markets operate within probabilistic frameworks, not deterministic outcomes. The disciplined practitioner identifies structural conditions, defines risk parameters, and executes only when probability and asymmetry align."}].map((s,i)=>(
        <div key={i} style={{ marginBottom:22 }}>
          ...
          {s.b.split("\n\n").map((p,j)=><p key={j} style={{ color:C.textMuted,lineHeight:1.85,fontSize:14,paddingLeft:13,marginBottom:8 }}>{p}</p>)}
        </div>
      ))}
  ...
  return (
    <div className="fi">
      <div style={{ marginBottom:28 }}>
        ...
        <p style={{ color:C.textMuted,fontSize:14 }}>Upload a chart for institutional-grade structural analysis using the Fortitude Market Framework.</p>
      </div>
        ...
        <div>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
            {[["Asset Class",<select className="inp" value={asset} onChange={e=>setAsset(e.target.value)}>{["forex","crypto","indices","commodities","equities"].map(o=><option key={o} value={o}>{o.charAt(0).toUpperCase()+o.slice(1)}</option>)}</select>],["Timeframe",<select className="inp" value={tf} onChange={e=>setTf(e.target.value)}>{["M1","M5","M15","M30","H1","H4","D1","W1","MN"].map(o=><option key={o}>{o}</option>)}</select>],["Optional Thesis",<textarea className="inp" value={thesis} onChange={e=>setThesis(e.target.value)} placeholder="Describe your structural observation..." style={{ resize:"vertical",minHeight:80 }}/>]].map(([label,el])=>(
              <div key={label}><label style={{ fontSize:11,color:C.textDim,display:"block",marginBottom:6,letterSpacing:".07em",textTransform:"uppercase" }}>{label}</label>{el}</div>
            ))}
  ...
  return (
    <div>
      <div style={{ marginBottom:16 }}>
        ...
        <div style={{ fontSize:13, color:C.textMuted }}>Your credentials are encrypted end-to-end. Fortitude requests read-only access only.</div>
      </div>
  ...
  return (
    <div className="fi">
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <p style={{ color:C.textMuted, fontSize:14 }}>Connect multiple trading accounts for live sync, or import historical data via CSV.</p>
      </div>
          ...
          {showConnect ? (
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))", gap:14, alignItems:"start" }}>
              ...
              {activeConn ? (
                ...
                <div style={{ textAlign:"center", padding:"60px 24px", color:C.textMuted, fontSize:14 }}>
...
const Coach = () => {
  ...
  if(view==="session") {
    ...
    return (
      <div className="fi" style={{ display:"flex",flexDirection:"column",height:"100%",minHeight:0 }}>
    ...
    <div className="mc" style={{ borderColor:C.accentDim }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:10,marginBottom:12 }}>
        {[
          ...
          <div key={f.k}>
            <label style={{ fontSize:11,color:C.textDim,display:"block",marginBottom:5,letterSpacing:".07em",textTransform:"uppercase" }}>{f.l}</label>
        ...
        <div>
          <label style={{ fontSize:11,color:C.textDim,display:"block",marginBottom:5,letterSpacing:".07em",textTransform:"uppercase" }}>Stop Condition</label>
        ...
        <div>
          <label style={{ fontSize:11,color:C.textDim,display:"block",marginBottom:5,letterSpacing:".07em",textTransform:"uppercase" }}>Session Duration (minutes)</label>
    ...
    <div className="fi">
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:10,marginBottom:14 }}>
        <div className="mc" style={{ borderColor:tone.color,background:`${tone.color}08`,padding:16 }}>
          ...
          <div style={{ fontSize:13,color:C.textMuted,lineHeight:1.7 }}>{tone.desc}</div>
      ...
      <div className="mc" style={{ marginBottom:18,borderColor:pdiMeta.color,background:`linear-gradient(135deg,rgba(13,16,24,.92),${pdiMeta.color}08)` }}>
        <div style={{ display:"flex",alignItems:"center",gap:28 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <p style={{ fontSize:14,color:C.textMuted,lineHeight:1.8,maxWidth:480 }}>
            ...
            return (
              <div key={i} className="mc" style={{ borderLeft:`3px solid ${priorityColor}`,display:"flex",gap:16 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <p style={{ fontSize:14,color:C.textMuted,lineHeight:1.8 }}>{r.body}</p>
                </div>
              </div>
            );
          })}
...
const Education = ({ currentTier = "45" }) => {
  ...
  if (view === "library") return (
    <div className="fi">
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: C.textMuted, fontSize: 14 }}>155 real lessons across 5 courses. Structured progression from foundation to advanced institutional methodology.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(290px,1fr))", gap: 14 }}>
        {COURSE_DATA.map(course => {
          const prog = getCourseProgress(course, completedIds);
          const isPurchase = course.tier === "purchase";
          const isMentorship = course.tier === "mentorship";
          const locked = isPurchase || isMentorship;
          // Purchase courses require active membership (not free tier)
          const noMembership = isPurchase && (currentTier === "free");
          const cardView = isMentorship ? "apply" : noMembership ? "membership-required" : isPurchase ? "checkout" : "course";
          return (
            <div key={course.id} className="mc"
              style={{ cursor: "pointer", opacity: locked ? .82 : 1, padding: 0, overflow: "hidden", transition: "all .2s", border: `1px solid ${C.border}` }}
              onClick={() => { setActiveCourseId(course.id); setView(cardView); }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = `${course.color}55`; e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = `0 8px 28px ${course.color}12`; }}
              ...
              <div style={{ padding: "16px 16px 14px" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", gap: 6, marginBottom: 7, flexWrap: "wrap" }}>
                      ...
                      {isPurchase && <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 3, background: "rgba(233,30,167,.1)", color: C.pink, border: "1px solid rgba(233,30,167,.2)" }}>PURCHASE</span>}
                      {isMentorship && <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 3, background: "rgba(212,175,55,.1)", color: "#d4af37", border: "1px solid rgba(212,175,55,.25)" }}>BY APPLICATION</span>}
                    </div>
                  ...
                  {/* Mini donut / mentorship icon */}
                  {isMentorship ? (
                    <div style={{ width: 42, height: 42, borderRadius: "50%", background: "rgba(212,175,55,.1)", border: "1px solid rgba(212,175,55,.25)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#d4af37" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                    </div>
                  ) : (
                ...
                <p style={{ fontSize: 13, color: C.textMuted, lineHeight: 1.7, marginBottom: 12, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{course.description}</p>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 11, color: C.textDim }}>{isMentorship ? `${course.duration} · Cohort-based` : `${prog.done}/${prog.total} lessons · ${course.duration}`}</span>
                  <span style={{ fontSize: 11, color: locked ? C.pink : course.color, fontWeight: 500 }}>
                    {isMentorship ? "Apply Now →" : isPurchase ? "Buy $495 →" : prog.pct === 0 ? "Start →" : prog.pct === 100 ? "Review →" : "Continue →"}
                  </span>
                </div>
              </div>
            </div>
  ...
  // ── Membership required ──
  if (view === "membership-required" && activeCourse) return (
    <MembershipRequired
      course={activeCourse}
      onBack={() => setView("library")}
      onGoToPricing={() => { window.__fortitudeNav && window.__fortitudeNav("pricing"); }}
    />
  );
  ...
  // ── Course checkout ──
  if (view === "checkout" && activeCourse) return (
    <CourseCheckout
      course={activeCourse}
      currentTier={currentTier}
      onBack={() => setView("library")}
      onPurchaseComplete={() => { setView("course"); }}
    />
  );
      ...
      <div style={{ marginBottom:16, flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            ...
            <p style={{ color:C.textMuted, fontSize:14 }}>High-impact macro events · Filter by currency and impact level · Data via TradingView</p>
          </div>
...
const Community = ({ user }) => {
  ...
  const displayName = user ? ((user.first_name||"")+" "+(user.last_name||"")).trim() || user.email?.split("@")[0] : "Member";
  const ME = { u:displayName.replace(" ","."), tag:"Member", tc:"tb", avatar:(user?.first_name?.[0]||"U").toUpperCase() };
  ...
  return (
    <div className="fi community-chat-wrap" style={{ display:"flex", flexDirection:"column", height:"100%", maxHeight:820,minHeight:0 }}>
      ...
      <div style={{ marginBottom:14, flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            ...
            <p style={{ color:C.textMuted,fontSize:14 }}>Professional market discussion. Structural analysis only.</p>
          </div>
...
const NARRATIVES = [
  { id:"hawkish-fed",  label:"Hawkish Fed",           strength:85, trend:"Peaking",  color:"#e91ea7", icon:"🏦", desc:"Higher for longer. Rate cuts pushed to Q3+" },
  { id:"geopolitical", label:"Geopolitical Risk",      strength:78, trend:"Building", color:"#ff6b35", icon:"⚡", desc:"Red Sea + multiple active conflict zones" },
  { id:"disinflation", label:"Disinflation Trend",     strength:62, trend:"Building", color:"#29a8ff", icon:"📉", desc:"Global inflation falling, still above target" },
  { id:"recession",    label:"Recession Risk",         strength:54, trend:"Building", color:"#d4af37", icon:"⚠️", desc:"IMF downgrade + PMI weakness globally" },
  { id:"carry-unwind", label:"Carry Trade Unwind",     strength:48, trend:"Building", color:"#e91ea7", icon:"🔄", desc:"BOJ normalisation threatening $4T carry trade" },
  { id:"usd-strength", label:"USD Strength",           strength:72, trend:"Peaking",  color:"#29a8ff", icon:"💵", desc:"Hawkish Fed + safe haven demand" },
];

const ASSET_DATA = [
  { asset:"Gold",      sym:"XAU/USD", bias:1,  score:7, icon:"🥇", reason:"Safe haven + disinflation dominant",  change:"+1.2%" },
  { asset:"US Dollar", sym:"DXY",     bias:1,  score:8, icon:"💵", reason:"Hawkish Fed + global risk-off",        change:"+0.7%" },
  { asset:"Oil",       sym:"Brent",   bias:1,  score:6, icon:"🛢️", reason:"Red Sea supply route risk",           change:"+3.8%" },
  { asset:"S&P 500",   sym:"SPX",     bias:-1, score:6, icon:"📊", reason:"Higher rates + growth concerns",       change:"-0.9%" },
  { asset:"10Y Bond",  sym:"US10Y",   bias:0,  score:4, icon:"📄", reason:"Conflicting inflation vs growth",      change:"-4bp"  },
  { asset:"EUR/USD",   sym:"EURUSD",  bias:-1, score:6, icon:"💶", reason:"ECB dovish vs Fed hawkish gap",        change:"-0.6%" },
  { asset:"USD/JPY",   sym:"USDJPY",  bias:-1, score:8, icon:"🇯🇵", reason:"BOJ hawkish — carry unwind risk",    change:"-2.4%" },
  { asset:"Bitcoin",   sym:"BTC/USD", bias:-1, score:5, icon:"₿",  reason:"Risk-off environment + USD strength", change:"-3.1%" },
];

const CAT_ICONS = {
  "Monetary Policy":"🏦", "Inflation Data":"📊", "Economic Data":"📈",
  "Geopolitical Conflict":"⚡", "War / Military":"🛡️", "Energy / Commodities":"🛢️",
  "Banking / Financial Stability":"🏛️", "Fiscal Policy":"📋",
  "Trade / Tariffs":"🔄", "Regulation":"⚖️", "Crypto / Digital Assets":"₿",
};

const REGIME_META = {
  "Risk-On":      { color:"#29a8ff", bg:"rgba(41,168,255,.1)",   icon:"🟢", label:"Risk-On"      },
  "Risk-Off":     { color:"#e91ea7", bg:"rgba(233,30,167,.1)",   icon:"🔴", label:"Risk-Off"     },
  "Deflationary": { color:"#d4af37", bg:"rgba(212,175,55,.1)",   icon:"🟡", label:"Deflationary" },
  "Inflationary": { color:"#ff6b35", bg:"rgba(255,107,53,.1)",   icon:"🟠", label:"Inflationary" },
};
...
const QuantResearchAnalyst = ({ currentTier, setPage }) => {
  ...
  const cats = ["All","Monetary Policy","Inflation Data","Economic Data","Geopolitical Conflict","Energy / Commodities"];
  const filtered = SAMPLE_EVENTS.filter(e => (filterCat === "All" || e.category === filterCat) && e.impactScore >= minScore);
          ...
          <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", background:"linear-gradient(to bottom, transparent 0%, rgba(6,8,16,.96) 60%)" }}>
            <div style={{ textAlign:"center", padding:"28px 36px", background:"rgba(13,16,24,.97)", border:"1px solid rgba(233,30,167,.25)", borderRadius:12, maxWidth:400 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:20, textAlign:"left" }}>
                {["⚡ Live intelligence feed","🗺️ Asset impact heatmap","🧠 AI event analysis","📡 Narrative tracker","📊 Daily & weekly reports","🔔 Smart alert system"].map(f => (
  ...
  return (
    <div className="fi">
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ display:"flex", gap:0, background:"rgba(13,16,24,.8)", border:"1px solid rgba(255,255,255,.06)", borderRadius:6, padding:2 }}>
            {[["📈","Trader"],["💼","Investor"]].map(([icon,label],i) => (
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))", gap:8, marginBottom:18 }}>
        {[
          { icon:"⚡", label:"Active Events",  value:SAMPLE_EVENTS.length,                                    color:C.accent },
          { icon:"🔴", label:"Critical (9-10)", value:SAMPLE_EVENTS.filter(e=>e.impactScore>=9).length,        color:"#e91ea7" },
          { icon:"🟠", label:"High (7-8)",      value:SAMPLE_EVENTS.filter(e=>e.impactScore>=7&&e.impactScore<9).length, color:"#ff6b35" },
          { icon:"📡", label:"Narratives",      value:NARRATIVES.length,                                       color:C.accent },
        ].map(s => (
      ...
      <div style={{ display:"flex", gap:2, marginBottom:18, borderBottom:"1px solid rgba(255,255,255,.06)", overflowX:"auto", scrollbarWidth:"none" }}>
        {[["feed","⚡ Feed"],["heatmap","🗺️ Heatmap"],["narratives","📡 Narratives"],["reports","📄 Reports"],["alerts","🔔 Alerts"]].map(([id,label]) => (
            ...
            <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
              {cats.map(c => (
                <button key={c} onClick={()=>setFilterCat(c)} style={{ padding:"5px 12px", borderRadius:5, fontSize:11, cursor:"pointer", background:filterCat===c?"rgba(41,168,255,.15)":"rgba(255,255,255,.03)", border:`1px solid ${filterCat===c?"rgba(41,168,255,.4)":"rgba(255,255,255,.06)"}`, color:filterCat===c?C.accent:C.textDim, transition:"all .15s" }}>
                  {c === "All" ? "All" : (CAT_ICONS[c] || "📊") + " " + c.replace(" / "," / ")}
                </button>
              ))}
            </div>
                  ...
                  <div style={{ background:"rgba(13,16,24,.95)", border:`1px solid ${statusColor(activeEvent.status)}50`, borderRadius:12, padding:"20px 22px" }}>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:16 }}>
                      <div style={{ display:"flex", gap:10, alignItems:"flex-start", flex:1, paddingRight:12 }}>
                        <div style={{ fontSize:28 }}>{CAT_ICONS[activeEvent.category] || "📊"}</div>
                        <div>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:16 }}>
                      {[
                        { label:"Impact",     value:`${activeEvent.impactScore}/10`, color:scoreColor(activeEvent.impactScore), icon:"⚡" },
                        { label:"Confidence", value:activeEvent.confidence,          color:C.accent,                           icon:"🎯" },
                        { label:"Urgency",    value:activeEvent.urgency,             color:activeEvent.urgency==="High"?C.pink:C.gold, icon:"⏱️" },
                      ].map(m => (
                    ...
                    <div style={{ display:"flex", gap:8, marginBottom:14 }}>
                      <div style={{ flex:1, padding:"10px 12px", background:regime.bg, border:`1px solid ${regime.color}40`, borderRadius:7, display:"flex", alignItems:"center", gap:8 }}>
                        <span style={{ fontSize:16 }}>{regime.icon}</span>
                        <div>
                    ...
                    {[
                      { icon:"📋", label:"What Happened",    text:activeEvent.whatHappened   },
                      { icon:"💡", label:"Why It Matters",   text:activeEvent.whyItMatters   },
                      { icon:"⚡", label:"Immediate Impact", text:activeEvent.immediateImpact },
                      { icon:"🔄", label:"Secondary Effects",text:activeEvent.secondaryEffects},
                    ].map(s => (
                    ...
                    <div style={{ marginBottom:14, padding:"12px 14px", background:"rgba(212,175,55,.05)", border:"1px solid rgba(212,175,55,.2)", borderRadius:8 }}>
                      ...
                      <div style={{ marginBottom:8 }}>
                        <div style={{ fontSize:10, color:C.textDim, marginBottom:3 }}>👁️ WATCH NEXT</div>
                        <div style={{ fontSize:12, color:C.textMuted, lineHeight:1.7 }}>{activeEvent.watchNext}</div>
                      </div>
                      <div>
                        <div style={{ fontSize:10, color:C.textDim, marginBottom:3 }}>🚫 INVALIDATION</div>
                    ...
                    <div style={{ marginTop:14, fontSize:10, color:C.textDim, fontStyle:"italic", lineHeight:1.6 }}>
                      ⚠️ Market intelligence only — not financial advice. Fact is separated from interpretation throughout.
                    </div>
                  </div>
                </div>
              );
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:10 }}>
            {ASSET_DATA.map(a => (
              <div key={a.asset} style={{ background:"rgba(13,16,24,.85)", border:`1px solid ${biasColor(a.bias)}30`, borderRadius:10, padding:"16px 16px", transition:"all .15s" }}>
                {/* Icon + name */}
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                  <div style={{ width:36, height:36, borderRadius:8, background:`${biasColor(a.bias)}12`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>{a.icon}</div>
                  <div>
        ...
        <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {NARRATIVES.sort((a,b) => b.strength - a.strength).map(n => {
              ...
              const trendIcon = n.trend==="Building"?"📈":n.trend==="Peaking"?"🔔":"📉";
              return (
                <div key={n.id} style={{ background:"rgba(13,16,24,.85)", border:`1px solid ${n.color}25`, borderRadius:10, padding:"16px 18px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                    <div style={{ display:"flex", gap:12, alignItems:"center", flex:1 }}>
                      <div style={{ width:40, height:40, borderRadius:8, background:`${n.color}15`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>{n.icon}</div>
                      <div>
      ...
      {activeTab === "reports" && (
        <div>
          {[
            { icon:"🌅", title:"Morning Macro Brief", time:"07:00 GMT · Today", type:"Daily", color:C.accent,
            ...
            { icon:"⚡", title:"Intraday Update",     time:"13:30 GMT · Today", type:"Intraday", color:"#ff6b35",
            ...
            { icon:"📊", title:"Weekly Narrative Report — W12", time:"Friday · This Week", type:"Weekly", color:"#d4af37",
            ...
            <div key={r.title} style={{ background:"rgba(13,16,24,.85)", border:`1px solid rgba(255,255,255,.06)`, borderRadius:10, padding:"18px 20px", marginBottom:10 }}>
              <div style={{ display:"flex", gap:14, alignItems:"flex-start" }}>
                <div style={{ width:44, height:44, borderRadius:9, background:`${r.color}12`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>{r.icon}</div>
                <div style={{ flex:1 }}>
          ...
          <div style={{ background:"rgba(13,16,24,.85)", border:"1px solid rgba(255,255,255,.06)", borderRadius:10, padding:"20px 22px", marginBottom:16 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:16 }}>
              ...
              <div>
                ...
                {[["🔔","Platform Notifications",true],["📧","Email Alerts","coming soon"]].map(([icon,label,active]) => (
      ...
      <div style={{ marginTop:20, padding:"10px 14px", background:"rgba(13,16,24,.6)", border:"1px solid rgba(255,255,255,.04)", borderRadius:6, fontSize:11, color:C.textDim }}>
        ⚠️ Market intelligence only — not financial advice. Fact is separated from interpretation. For educational purposes only.
      </div>
    </div>
  );
};
