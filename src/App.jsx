...
const TradingSessionsMap = () => {
  ...
  const fmtCity = (tz) => {
    try {
      return new Intl.DateTimeFormat("en-GB", { timeZone:tz, hour:"2-digit", minute:"2-digit", second:"2-digit" }).format(now);
    } catch { return "--:--:--"; }
  };
  ...
  const SESSIONS = [
    { id:"tokyo",  label:"Tokyo",    tz:"Asia/Tokyo",       open:0,    close:6.5,  color:"#7b68ee", exchange:"TSE" },
    { id:"london", label:"London",   tz:"Europe/London",    open:7,    close:16,   color:"#29a8ff", exchange:"LSE" },
    { id:"newyork",label:"New York", tz:"America/New_York", open:13.5, close:20,   color:"#e91ea7", exchange:"NYSE" },
  ];

  const KZ_WINDOWS = [
    { label:"London Open",    open:7,    close:9,    color:"#29a8ff", desc:"Asian range sweep · Stop hunts · First directional move" },
    { label:"LDN–NY Overlap", open:13.5, close:16.5, color:"#d4af37", desc:"Highest global liquidity · XAUUSD priority · Institutional flow" },
    { label:"NY Open",        open:13.5, close:15.5, color:"#e91ea7", desc:"Macro data reactions · Institutional entries · Trend continuation" },
    { label:"London Close",   open:16,   close:17,   color:"#7b68ee", desc:"Position closing · Liquidity spikes · Reversals" },
  ];

  const getState = (s) => {
    if (isFxWeekend) return { isOpen:false, pct:0, countdown:"Weekend", cityTime:fmtCity(s.tz) };
    const isOpen = utc >= s.open && utc < s.close;
    const span = (s.close - s.open) * 3600;
    const elapsed = (utc - s.open) * 3600;
    const remain = (s.close - utc) * 3600;
    const tillOpn = ((s.open - utc + 24) % 24) * 3600;
    return { isOpen, pct:isOpen ? Math.min(100,(elapsed/span)*100) : 0, countdown:isOpen ? fmtSecs(remain) : fmtSecs(tillOpn), cityTime:fmtCity(s.tz) };
  };

  const londonOpen = !isFxWeekend && utc >= 7 && utc < 16;
  const nyOpen     = !isFxWeekend && utc >= 13.5 && utc < 20;
  const isKillzone = londonOpen && nyOpen;

  return (
    <div style={{ background:"rgba(6,8,16,.9)", border:"1px solid rgba(255,255,255,.07)", borderRadius:10, overflow:"hidden" }}>
      <div style={{ padding:"12px 18px", borderBottom:"1px solid rgba(255,255,255,.06)", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <IC n="globe" s={14} c={C.accent}/>
          <span style={{ fontSize:11, fontWeight:600, color:C.text, letterSpacing:".08em", textTransform:"uppercase" }}>Trading Sessions</span>
          {isKillzone && <span style={{ fontSize:10, padding:"2px 8px", borderRadius:3, background:"rgba(212,175,55,.15)", color:"#d4af37", border:"1px solid rgba(212,175,55,.3)", fontWeight:600 }}>LDN–NY OVERLAP</span>}
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          {isFxWeekend
            ? <span style={{ fontSize:10, color:C.textDim }}>Markets closed</span>
            : <><div style={{ width:6,height:6,borderRadius:"50%",background:"#29ff88",boxShadow:"0 0 6px #29ff88"}}/><span style={{ fontSize:10, color:C.textDim }}>Live</span></>
          }
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:0 }}>
        {SESSIONS.map((s, i) => {
          const st = getState(s);
          return (
            <div key={s.id} style={{ padding:"16px 18px", borderRight:i<2?"1px solid rgba(255,255,255,.05)":"none" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:4 }}>
                    <div style={{ width:7,height:7,borderRadius:"50%", background:st.isOpen?s.color:"#3a3f52", boxShadow:st.isOpen?`0 0 8px ${s.color}80`:"none" }}/>
                    <span style={{ fontSize:13, fontWeight:600, color:C.text }}>{s.label}</span>
                  </div>
                  <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:22, fontWeight:700, color:st.isOpen?s.color:C.textDim, letterSpacing:".03em", lineHeight:1 }}>{st.cityTime}</div>
                </div>
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:10, padding:"3px 8px", borderRadius:3, background:st.isOpen?`${s.color}18`:"rgba(255,255,255,.04)", color:st.isOpen?s.color:C.textDim, border:`1px solid ${st.isOpen?s.color+"40":"rgba(255,255,255,.06)"}`, fontWeight:600, marginBottom:4 }}>{st.isOpen?"OPEN":"CLOSED"}</div>
                  <div style={{ fontSize:10, color:C.textDim }}>{s.exchange}</div>
                </div>
              </div>
              <div style={{ height:3, background:"rgba(255,255,255,.05)", borderRadius:2, marginBottom:8, overflow:"hidden" }}>
                <div style={{ height:"100%", width:`${st.pct}%`, background:s.color, borderRadius:2 }}/>
              </div>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <span style={{ fontSize:10, color:C.textDim }}>{isFxWeekend?"Weekend":st.isOpen?"Closes in":"Opens in"}</span>
                <span style={{ fontSize:11, color:st.isOpen?s.color:C.textDim, fontFamily:"JetBrains Mono,monospace" }}>{st.countdown}</span>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ borderTop:"1px solid rgba(255,255,255,.05)", padding:"10px 18px" }}>
        <div style={{ fontSize:10, color:C.textDim, letterSpacing:".07em", textTransform:"uppercase", marginBottom:8 }}>Key Liquidity Windows</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:8 }}>
          {KZ_WINDOWS.map(kz => {
            const isLive = !isFxWeekend && utc >= kz.open && utc < kz.close;
            return (
              <div key={kz.label} style={{ padding:"10px 12px", borderRadius:7, background:isLive?`${kz.color}0a`:"rgba(255,255,255,.02)", border:`1px solid ${isLive?kz.color+"40":"rgba(255,255,255,.05)"}` }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
                  <span style={{ fontSize:11, fontWeight:600, color:isLive?kz.color:C.textMuted }}>{kz.label}</span>
                  {isLive && <div style={{ width:5,height:5,borderRadius:"50%",background:kz.color,boxShadow:`0 0 5px ${kz.color}`}}/>}
                </div>
                <div style={{ fontSize:10, color:C.textDim, marginBottom:4, fontFamily:"JetBrains Mono,monospace" }}>{fmtH(kz.open)}–{fmtH(kz.close)} UTC</div>
                <div style={{ fontSize:10, color:C.textDim, lineHeight:1.5 }}>{kz.desc}</div>
                {isLive && <div style={{ fontSize:10, color:kz.color, marginTop:5, fontWeight:600 }}>ACTIVE NOW</div>}
              </div>
            );
          })}
        </div>
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", background:"rgba(41,168,255,.04)", border:"1px solid rgba(41,168,255,.12)", borderRadius:8 }}>
      <div style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
        ...
        <span style={{ fontSize:10, fontWeight:600, color:C.accent, letterSpacing:".08em", textTransform:"uppercase" }}>AI Insight</span>
      </div>
      ...
      <div className="dash-grid" style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) 300px", gap:14, alignItems:"start" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }} className="stats-grid">
            {[
              { l:"Analyses Completed", v:"47",       s:"This month",    c:C.accent,   icon:"activity" },
              { l:"Course Progress",    v:"68%",       s:"FMF Course",    c:C.gold,     icon:"edu" },
              { l:"Community Posts",    v:"12",        s:"Contributions", c:C.textMuted,icon:"comm" },
              { l:"PDI Score",          v:`${pdi}`,    s:pdiMeta.label,   c:pdiMeta.color, icon:"brain" },
            ].map(m => (
              <div key={m.l} className="mc" style={{ padding:"14px 14px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:8 }}>
                  <IC n={m.icon} s={13} c={m.c}/>
                  <div className="sl" style={{ margin:0, fontSize:10 }}>{m.l}</div>
                </div>
                <div className="mn" style={{ fontSize:22, color:m.c, fontWeight:400, lineHeight:1.1 }}>{m.v}</div>
                <div style={{ fontSize:10, color:C.textDim, marginTop:3 }}>{m.s}</div>
              </div>
            ))}
          </div>
          ...
          {(() => {
            ...
            const pf       = (() => { const gp=trades.filter(t=>t.win).reduce((a,t)=>a+parseFloat(t.result),0); const gl=Math.abs(trades.filter(t=>!t.win).reduce((a,t)=>a+parseFloat(t.result),0)); return gl>0?(gp/gl).toFixed(2):"∞"; })();
            const recent   = [...trades].sort((a,b)=>new Date(b.time)-new Date(a.time)).slice(0,5);
            return (
              <div className="mc" style={{ padding:"16px 20px" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:10, marginBottom:14 }}>
                  {[
                    ...
                    { l:"Profit Factor", v:pf,             c:parseFloat(pf)>=1?C.accent:C.pink },
                    { l:"Trades",        v:trades.length,  c:C.text },
                  ].map(m=>(
                    <div key={m.l} style={{ textAlign:"center", padding:"10px 6px", background:"rgba(13,16,24,.6)", borderRadius:6, border:`1px solid ${C.border}` }}>
                      ...
                      <div style={{ fontSize:9, color:C.textDim, marginTop:3, letterSpacing:".04em", textTransform:"uppercase" }}>{m.l}</div>
                    </div>
                  ))}
                </div>
