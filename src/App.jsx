...
const Login = ({ onLogin }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const data = await api.post(mode === "login" ? "/auth/login" : "/auth/register", { email, password: pass });
      if (data.success) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
      ...
      <div className="fi" style={{ position: "relative", zIndex: 1, width: "min(380px,92vw)", padding: "clamp(24px,5vw,40px)", background: "rgba(13,16,24,.93)", border: `1px solid ${C.border}`, borderRadius: 10, backdropFilter: "blur(16px)", boxShadow: "0 0 60px rgba(41,168,255,.07)" }}>
        ...
        {error && <div style={{ marginBottom:14, padding:"10px 14px", background:"rgba(233,30,167,.08)", border:`1px solid ${C.pink}`, borderRadius:6, fontSize:12, color:C.pink }}>{error}</div>}
...
export default function App() {
  ...
  const handleLogout = () => {
    ...
    setSubStatus("inactive"); setPage("dashboard");
  };
