    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
      ...
      <div className="fi" style={{ position: "relative", zIndex: 1, width: "min(380px,92vw)", padding: "clamp(24px,5vw,40px)", background: "rgba(13,16,24,.93)", border: `1px solid ${C.border}`, borderRadius: 10, backdropFilter: "blur(16px)", boxShadow: "0 0 60px rgba(41,168,255,.07)" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 10, color: C.textMuted, letterSpacing: ".18em", textTransform: "uppercase", marginTop: 6, fontWeight: 500 }}>Market Intelligence Platform</div>
        </div>
...
const Pricing = ({ currentTier, setPage, onUpgrade, subStatus, setSubStatus }) => {
  ...
  const TIER_FEATURES = {
    free:   ["Introduction to Trading Course","Beginner Crypto Course","Dashboard overview (limited)","Platform login"],
    "45":   ["Full Dashboard","Market Intelligence","AI Chart Analysis (5/day)","Community Chat","Advanced Trading Course (purchase)","5-Day Workshop (purchase)","Market Updates"],
  ...
  const COURSE_HIGHLIGHTS = [
    { label: "Introduction Course",  tier: "Free",  color: C.accent,  note: "Included with all plans" },
    { label: "Beginners Course",     tier: "Core",  color: C.accent,  note: "Included in Core ($45/mo)" },
    { label: "Crypto Course",        tier: "Core",  color: C.accent,  note: "Included in Core ($45/mo)" },
    { label: "Advanced Course",      tier: "Pro",   color: C.gold,    note: "Purchase $495 · Core+ required" },
    { label: "5-Day Workshop",       tier: "Pro",   color: C.gold,    note: "Purchase $495 · Core+ required" },
    { label: "Elite Mentorship",     tier: "Apply", color: "#d4af37", note: "By application only · $2,995" },
  ];
  ...
  if (trialDone) return (
    <div className="fi" style={{ maxWidth: 560, margin: "0 auto", padding: "40px 0" }}>
      <div className="mc" style={{ textAlign: "center", padding: "44px 32px" }}>
        ...
        <div style={{ marginTop: 12, fontSize: 11, color: C.textDim }}>Confirmation sent to j.harrison@email.com</div>
      </div>
    </div>
  );
    ...
    <div className="fi">
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:10, marginBottom:24 }}>
        {COURSE_HIGHLIGHTS.map(c => {
          const accessible = c.tier !== "Apply" && (currentTier !== "free" || c.tier === "Free");
          return (
            <div key={c.label} className="mc" style={{ padding:"14px 16px", opacity: c.tier === "Apply" ? 1 : accessible ? 1 : 0.65 }}>
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10 }}>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:5 }}>
                    <span style={{ fontSize:9, fontWeight:700, letterSpacing:".08em", padding:"2px 7px", borderRadius:3, background:`${c.color}15`, color:c.color, border:`1px solid ${c.color}30` }}>{c.tier.toUpperCase()}</span>
                    {!accessible && c.tier !== "Apply" && <span style={{ fontSize:9, color:C.textDim }}>Subscription required</span>}
                  </div>
                  <div style={{ fontSize:13, color:C.text, fontWeight:500, marginBottom:3 }}>{c.label}</div>
                  <div style={{ fontSize:11, color:C.textDim }}>{c.note}</div>
                </div>
                <button className="btn bg" style={{ fontSize:10, padding:"5px 10px", flexShrink:0 }}
                  onClick={() => setPage("education")}>
                  {c.tier === "Apply" ? "Apply" : c.tier === "Free" || accessible ? "Access" : "Unlock"}
                </button>
              </div>
            </div>
          );
  ...
  return (
    <div className="fi">
      ...
      <div style={{ display:"flex",gap:2,marginBottom:16,borderBottom:`1px solid ${C.border}`,paddingBottom:0,overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none",msOverflowStyle:"none" }}>
        {[{id:"subscription",label:"Subscription"},{id:"usage",label:"AI Usage & Limits"},{id:"security",label:"Security"},{id:"affiliate",label:"Affiliate Programme"}].map(t=>(
        ...
        <div className="fi">
          <div className="mc" style={{ marginBottom:14 }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12 }}>
              <div><label style={{ fontSize:11,color:C.textDim,display:"block",marginBottom:6,letterSpacing:".07em",textTransform:"uppercase" }}>Email Address</label><input className="inp" defaultValue="j.harrison@email.com"/></div>
              <div><label style={{ fontSize:11,color:C.textDim,display:"block",marginBottom:6,letterSpacing:".07em",textTransform:"uppercase" }}>New Password</label><input className="inp" type="password" placeholder="••••••••"/></div>
            </div>
  ...
  return (
    <div className="fi">
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:10, marginBottom:20 }}>
        {[
          { l:"Total Earned",    v:`$${totalEarned.toFixed(2)}`,     c:C.gold,   s:"Paid out" },
          { l:"Pending",         v:`$${pendingEarnings.toFixed(2)}`, c:C.accent, s:"Next payout: 1 Apr" },
          { l:"Active Referrals",v:activeCount,                       c:C.accent, s:`${trialCount} on trial` },
          { l:"Conversion Rate", v:"71%",                             c:C.accent, s:"vs 58% platform avg" },
          { l:"Commission Rate", v:"20%",                             c:"#d4af37", s:"On first payment" },
        ].map(m=>(
      ...
      <div style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) min(320px,42%)", gap:16, marginBottom:20 }}>
        ...
        <div>
          <div className="mc" style={{ padding:"20px 20px", border:"1px solid rgba(212,175,55,.15)", marginBottom:14 }}>
            <div className="sl" style={{ marginBottom:14, color:"#d4af37" }}>Commission Structure</div>
            <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
              {[
                { tier:"Core ($45/mo)",    rate:"20%",  amount:"$9/mo",  color:C.accent },
                { tier:"Pro ($65/mo)",     rate:"20%",  amount:"$13/mo", color:C.accent },
                { tier:"Elite ($95/mo)",   rate:"20%",  amount:"$19/mo", color:"#d4af37" },
                { tier:"Advanced Course",  rate:"10%",  amount:"$49.50", color:C.gold },
                { tier:"Workshop",         rate:"10%",  amount:"$49.50", color:C.gold },
                { tier:"Mentorship",       rate:"5%",   amount:"$149.75",color:"#d4af37" },
              ].map((r,i,arr) => (
                <div key={r.tier} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom: i<arr.length-1?`1px solid ${C.border}`:"none" }}>
                  <div>
                    ...
                    <div style={{ fontSize:10, color:C.textDim }}>{r.rate} of first payment</div>
                  </div>
            ...
            <div style={{ marginTop:14, padding:"10px 12px", background:"rgba(212,175,55,.05)", border:"1px solid rgba(212,175,55,.12)", borderRadius:6, fontSize:11, color:C.textDim, lineHeight:1.7 }}>
              Commissions are calculated on the first paid billing cycle only. Subscriptions that cancel during trial generate no commission. Payouts processed monthly on the 1st.
            </div>
          </div>
      ...
      <div className="mc" style={{ padding:"20px 22px" }}>
              ...
              {filteredRefs.length === 0 ? (
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td className="mn" style={{ textAlign:"right", color: r.commission > 0 ? C.gold : C.textDim }}>{r.commission > 0 ? `$${r.commission.toFixed(2)}` : "—"}</td>
        ...
        <div style={{ display:"flex", justifyContent:"flex-end", gap:24, marginTop:14, paddingTop:12, borderTop:`1px solid ${C.border}` }}>
          <div style={{ textAlign:"right" }}>
            ...
            <div className="mn" style={{ fontSize:14, color:C.gold }}>${totalEarned.toFixed(2)}</div>
          </div>
          <div style={{ textAlign:"right" }}>
      ...
      <div style={{ marginTop:14, padding:"12px 16px", background:"rgba(13,16,24,.6)", border:`1px solid ${C.border}`, borderRadius:6 }}>
        <div style={{ fontSize:11, color:C.textDim, lineHeight:1.9 }}>
          <strong style={{ color:C.textMuted }}>Affiliate Programme Terms:</strong> You earn a one-time commission on the first successful payment made by each referred user. Commissions are not earned on trial periods or subscriptions that cancel before first billing. Self-referrals are not permitted. Payouts processed monthly — minimum $50 balance required. Fortitude reserves the right to withhold commissions for fraudulent referral activity. Full terms available on request.
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const ALWAYS_OPEN = new Set(["account","pricing","education","affiliate"]);
